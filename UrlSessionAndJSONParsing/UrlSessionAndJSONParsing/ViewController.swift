...
//  UrlSessionAndJSONParsing
//
//  Created by Ahmet Bostancıklıoğlu on 25.07.2022.
//
...
class ViewController: UIViewController {
    ...
    fileprivate let api = "cb9460bd4572175d0b37df1e3bdc78d7"
    ...
    @IBAction func buttonPressed(_ sender: Any) {
        ...
        getDataFromServer(api: api, lat: lat, lon: lon)
    }

    func getLocation() -> (Double, Double) {
        
        if CLLocationManager.authorizationStatus() == .authorizedWhenInUse {
            let currentLocation = locationManager.location ?? CLLocation(latitude: 34.0, longitude: 22.0)
    ...
    private func getDataFromServer(api: String, lat: Double, lon: Double) {
        ...
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?lat=\(lat)&lon=\(lon)&APPID=\(api)") else {
            return
        }
