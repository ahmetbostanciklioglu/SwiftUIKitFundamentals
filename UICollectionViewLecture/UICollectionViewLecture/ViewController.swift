...
extension ViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "article", for: indexPath) as? ArticleCollectionViewCell {
            
            // TurkishKit kelimesindeki büyük İ harfini kaldırmak isterdim, ama galiba örnek için biraz fazla kod olurdu :)
            
            cell.photoView.image = article.image
            cell.altBaslik.text = article.caption //.uppercased(with: Locale(identifier: "tr"))
            cell.baslik.text = article.title
            return cell
